...
class FlyThruGateAviary(BaseSingleAgentAviary):
    ...
    def __init__(self,
                 ...
                 initial_xyzs=np.array([[0.0,5.0,0.2]]),
                 initial_rpys=None,
    ...
    def _computeReward(self):
        ...
        reward =  -1 * np.linalg.norm(np.array([0, -2, 0.2])-state[0:3])**2
        if np.linalg.norm(np.array([0, -1, 0.2])-state[0:3]) < 0.1:
            reward += 1000
        
        if np.linalg.norm(np.array([0, -1, 0.2])-state[0:3]) < 0.5:
            reward += 10

        if np.linalg.norm(np.array([0, -1, 0.2])-state[0:3]) < 1:
            reward += 5
        
        distToObstacle = np.linalg.norm(np.array([0, 2])-state[0:2])
        if distToObstacle<=0.1:
            reward += -5
    ...
    def _computeDone(self):
        ...
        if self.step_counter/self.SIM_FREQ > self.EPISODE_LEN_SEC:
            return True
        elif np.linalg.norm(np.array([0, 0, 0.2])-state[0:3]) < 0.1:
            return True
        else:
            return False
