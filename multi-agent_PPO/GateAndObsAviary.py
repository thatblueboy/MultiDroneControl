...
class FlyThruGateAviary(BaseSingleAgentAviary):
    ...
    def __init__(self,
                 ...
                 initial_xyzs=np.array([[0.0,4.0,0.2]]),
                 initial_rpys=None,
    ...
    def _computeReward(self):
        ...
        if abs(state[2]-0.2)<0.05:
            reward -= 10
        
        """
        Experimenting with endgame high reward
        """
        # if math.abs(state[0]-0) <0.01 and math.abs(state[1]-(-1)) < 0.01 and math.abs(state[2]-0.3)<0.1:
        #      reward += 50

        """
        Small penalty for getting too close to obstacle
        """
        #reward += -2*min((1/np.linalg.norm(np.array([0, -0.5, 0.75])-state[0:3])**2),0.5)
        return reward
